...
// Atlas discipline optionId -> label mapping
const DISCIPLINE_OPTIONS = {
  '13ca9209-3358-4e07-a154-fedd0db60d53': 'Software Engineering',
  '4d772834-b9d7-4e39-8478-63e69973c156': 'Data',
}

// Extract a value from Atlas customAttributes array by attribute name
// Atlas stores selections as optionId references, not text values
function getCustomAttr(customAttributes, name) {
  ...
  if (!attr) return null
  // New API format: values is an array
  if (Array.isArray(attr.values) && attr.values.length > 0) {
    const val = attr.values[0]
    if (typeof val === 'object' && val.optionValue !== undefined) return val.optionValue
    return val
  }
  // Old format fallback: value or optionId at top level
  if (attr.value) return attr.value
  if (attr.optionId && DISCIPLINE_OPTIONS[attr.optionId]) {
    return DISCIPLINE_OPTIONS[attr.optionId]
  }
  return attr.optionId || null
}
