...
export async function fetchRoleDetail(id) {
  ...
  console.log('Atlas raw keys for', id, ':', Object.keys(result).join(', '));
  console.log('Atlas location/placeOfWork for', id, ':', JSON.stringify({ location: result.location, placeOfWork: result.placeOfWork, numberOfPositions: result.numberOfPositions, customAttributes: result.customAttributes }));
  return result
}
...
export function transformRole(raw) {
  ...
  return {
    ...
    numberOfPositions: raw.numberOfPositions || null,
    placeOfWork: raw.placeOfWork || raw.customAttributes?.placeOfWork || null,
  }
}
