...
function blocksToText(input) {
  ...
  if (typeof input === 'string') {
    ...
    if (input.startsWith('[') || input.startsWith('{')) {
      try {
        ...
        // Not JSON — could be HTML or plain text, return as-is
        return input
      }
    } else {
      return input
  ...
  return input
    .map((block) => {
      ...
      return [texts, childText].filter(Boolean).join('\n')
    })
    .filter(Boolean)
    .join('\n\n')
}
...
export function transformRole(raw) {
  ...
  const currency = raw.salaryCurrency || (salary && typeof salary === 'string' && salary.match(/[Â£â¬$]/) ? salary.match(/[Â£â¬$]/)[0] : '')

  return {
    id: raw.id,
    title: raw.jobRole || raw.title || 'Untitled role',
    slug: slugify(raw.jobRole || raw.title || raw.id),
    jobDescription: blocksToText(raw.jobDescription) || '',
