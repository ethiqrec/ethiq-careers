// Roles data layer â fetches from Atlas or falls back to local jobs.json
...
function formatLocation(location, workMode) {
  ...
  if (workMode === 'remote') return country ? `Remote Â· ${country}` : 'Remote'
  if (workMode === 'hybrid') return city ? `Hybrid Â· ${city}` : 'Hybrid'
  if (workMode === 'office') return city ? `On-site Â· ${city}` : 'On-site'
...
// Build the descriptor line: "Series A fintech Â· 40 people Â· remote-first EU Â· visa supported"
